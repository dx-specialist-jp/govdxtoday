...
import { buildGovSummaryPrompt, buildNewsFilterPrompt } from './prompts.js';
...
async function summarizeGovArticles(articles, model) {
  ...
  const prompt = buildGovSummaryPrompt(inputJson);
...
async function filterAndSummarizeNews(articles, model, maxCount = 5) {
  ...
  const prompt = buildNewsFilterPrompt(inputJson, maxCount);
