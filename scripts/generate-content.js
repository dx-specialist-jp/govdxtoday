...
const CLOUD_SOURCES = [
  ...
  { provider: 'さくらのクラウド',  name: 'さくらインターネット ニュース', url: 'https://www.sakura.ad.jp/corporate/feed/' },
...
const DEFAULT_RELEVANCE = '元記事の内容をご確認のうえ、所管業務への影響・対応要否のご検討をお願いします。';

// news_topics の sources 配列に格納する1記事分のエントリを組み立てる（複数箇所で共通利用）
function toSourceEntry(a) {
  return { name: outletName(a), url: a.url || '' };
}

// Google Alerts経由の記事は sourceName が「Google Alert: <検索キーワード>」という
// 検索カテゴリ名になっており、複数記事を1トピックに統合すると全ソースが同じラベルで
// 表示され、読者がどの記事がどの出典かを判別できなくなる。実際の配信元が分かるよう、
// その場合はホスト名を表示名として使う（政府公式・CSP公式RSS等はsourceNameが個別の
// フィード名なのでそのまま使う）
function outletName(a) {
  if (a.sourceName && !a.sourceName.startsWith('Google Alert:')) return a.sourceName;
  try {
...
// クエリ文字列・フラグメント違いだけの同一記事（例: ?iref=... の有無だけが異なるasahi.com記事）を
// 重複として扱うための正規化キー。実URLはそのまま保持し、比較用途にのみ使う
...
async function main() {
  ...
  // URL重複排除（クエリ文字列・フラグメント違いだけの同一記事も正規化して除外する）
  const seenUrls = new Set();
  ...
  const cloudItemsDeduped = cloudItemsRaw.filter((item) => {
    ...
    if (seenCloudUrls.has(key)) return false;
    seenCloudUrls.add(key);
    return true;
  });
